...
/**
 * MCP Tool: Get budget status — compare budgets vs actual spend per category.
 * Calls Transaction Service GET /api/transactions/budget-status
 */
@Component
@RequiredArgsConstructor
public class GetBudgetStatusTool implements McpTool {

    private static final Logger log = LoggerFactory.getLogger(GetBudgetStatusTool.class);
    ...
    public String getDescription() {
        return "Get the user's budget status for a given month. " +
               "Shows budget limits vs actual spending per category, percentage used, and whether over budget. " +
               "Use this when the user asks about their budget, if they're on track, or budget health.";
    }
    ...
    public McpToolResponse execute(McpToolCall call, String jwtToken) {
        try {
            ...
            log.info("Executing get_budget_status: userId={}, month={}, year={}", userId, month, year);

            String response = transactionServiceClient.get()
                    .uri(uriBuilder -> uriBuilder
                            .path("/api/transactions/budget-status")
            ...
            log.error("Failed to get budget status", e);
            return McpToolResponse.failure("Failed to retrieve budget status: " + e.getMessage());
        }
    }
}
